...
HEADING = re.compile(
    r"^\s*(\d{1,2}(?:\.\d{1,2}){0,2})\.?\s+([A-Z][a-zA-Z][\w\-,'&:?!()/. ]{1,60})\s*$"
)
...
KNOWN = re.compile(
    r"^\s*(Abstract|Introduction|Related Work|Conclusion|References|Acknowledgements|Appendix)\s*$",
    re.IGNORECASE,
)
    ...
    for page_number, page in enumerate(pages, start=1):
        ...
        while i < len(page_lines):
            ...
            if (m := HEADING.match(line)) and not BLACKLIST.match(m.group(2)):
                ...
                if (
                    t
                    and not BLACKLIST.match(t.group(1))
                    and accept(_parse_num(m.group(1)))
                ):
